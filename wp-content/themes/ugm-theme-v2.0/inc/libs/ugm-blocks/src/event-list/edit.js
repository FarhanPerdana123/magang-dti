	...
	return (
		<>
			...
			<div { ...blockProps }>
				{ posts && posts.length && (
					<Disabled>
						<div class="ugm-blocks-block-event-list">
							<div class={ "events events-"+layout+" column-"+columnCount }>
								{ posts.map( post => (
									<article class="post post-event event">
										<div class="event-date">
											<span>{ post.event_details.event_date }<strong>{ post.event_details.event_month }</strong></span>
										</div>
										<div class="post-content">
											<div class="post-title">
												<h3><a href={ post.link }>{ post.title.rendered }</a></h3>
												<span class="post-date">{ post.event_details.description }</span>
											</div>
										</div>
									</article>
								) ) }

...
$author_bio  = $author instanceof WP_User ? $author->description : '';
?>
<main id="primary" class="site-main ugm-archive">
	<div class="ugm-archive__container">
		...
		<nav class="ugm-archive__breadcrumb" aria-label="<?php esc_attr_e( 'Breadcrumb', 'ugm-faculty' ); ?>">
			...
			<span aria-current="page"><?php echo esc_html( $author_name ); ?></span>
		</nav>

		<header class="ugm-archive__header">

			<?php if ( $author instanceof WP_User ) : ?>
				<div class="ugm-author-profile">
					<div class="ugm-author-profile__avatar">
						<?php echo get_avatar( $author->ID, 80, '', esc_attr( $author_name ) ); ?>
					</div>
					<div class="ugm-author-profile__info">
						<h1 class="ugm-archive__title"><?php echo esc_html( $author_name ); ?></h1>
						<?php if ( $author_bio ) : ?>
							<p class="ugm-author-profile__bio"><?php echo wp_kses_post( $author_bio ); ?></p>
						<?php endif; ?>
					</div>
				</div>
			<?php else : ?>
				<h1 class="ugm-archive__title"><?php esc_html_e( 'Penulis', 'ugm-faculty' ); ?></h1>
			<?php endif; ?>
		...
		<?php else : ?>
			<p class="section-empty"><?php esc_html_e( 'Belum ada tulisan dari penulis ini.', 'ugm-faculty' ); ?></p>
		<?php endif; ?>
